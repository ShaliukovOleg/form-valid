...
// Формы валидности при проверке инпутов
const characterLimit = /^[a-zA-z_\d]{3,20}$/;
// const datePattern = /^(\d{2})\.(\d{2})\.(\d{4})$/;
// const paymentPattern = /^[\d]{3,20}$/;
...
function checkName() { // Проверка валидности имени
	switch (true) {
...
function checkSurName() { // Проверка валидности фамилии
	switch (true) {
...
function checkDate() { // Проверка валидности даты
	...
	let match = dateInput.value.match(datePattern);

	switch (true) {
	case dateInput.value.match(datePattern) === null: // Проверка на количество символов в строке
	...
	switch (true) {
	case (dateValue.getFullYear() == year && dateValue.getMonth() == month - 1 && dateValue.getDate() == day) === false: // Проверка на корректность символов
		fieldDate.classList.add('invalid');
		validText.classList.add('invalid');
		errorTextDate.textContent = errorFormat;
		break;
	case (dateValue > dateBefore) === false: // Не ниже 1940
		fieldDate.classList.add('invalid');
		validText.classList.add('invalid');
		errorTextDate.textContent = errorDateBelow;
		break;
	case (dateValue < dateNow) === false: // Не больше чем сейчас
		fieldDate.classList.add('invalid');
		validText.classList.add('invalid');
		errorTextDate.textContent = errorDateFuture;
		break;

	case (Math.ceil((dateNow - dateValue) / (1000 * 3600 * 24))) > 6574 === false: // проверка на совершеннолетие пользователя согласно вводным данным
		fieldDate.classList.add('invalid');
